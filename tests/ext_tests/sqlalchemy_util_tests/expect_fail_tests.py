...
@mark.xfail
@mark.sqla_utils
def test_fail_sqla_sqlite_in_memory_conn(sqla_sqlite_inmemory: sqlalchemy_utils.saSQLiteConnection):
    assert sqla_sqlite_inmemory is not None, ValueError("sqla_sqlite_inmemory cannot be None")
    assert not isinstance(sqla_sqlite_inmemory, sqlalchemy_utils.saSQLiteConnection), TypeError(f"sqla_sqlite_inmemory must be of type saSQLiteConnection, not ({type(sqla_sqlite_inmemory)})")
...
def test_fail_sqla_sqlite_engine(sqla_sqlite_engine: sa.Engine):
    ...
    assert not isinstance(sqla_sqlite_engine, sa.Engine), TypeError(f"sqla_sqlite_engine must be of type sqlalchemy.Engine. Got type: ({type(sqla_sqlite_engine)})")
