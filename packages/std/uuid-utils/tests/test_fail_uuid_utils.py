...
def test_fail_gen_uuid():
    assert isinstance(uuid_utils.gen_uuid(), str), (
...
def test_fail_trim_uuid():
    assert uuid_utils.trim_uuid(trim="one", in_uuid="This is a test") is not None, (
...
def test_fail_first_n_chars():
    assert uuid_utils.first_n_chars(first_n=15, in_uuid=None) is None, (
        "Failed assertion expected"
    )
